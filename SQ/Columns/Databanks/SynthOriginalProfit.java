...
public class SynthOriginalProfit extends DatabankColumn {

    public SynthOriginalProfit() {
        super("SynthOriginalProfit",
        ...
        setWidth(140);
        setTooltip("Net Profit obtenido en el re-backtest sobre la data original ejecutado por el Custom Analysis (CVSintetica).");
    }

    @Override
    public String getValue(ResultsGroup rg, String resultKey, byte direction, byte plType, byte sampleType) throws Exception {
        String key = "CA_OriginalProfit" + getSuffix(sampleType);
        Object v = rg.specialValues().get(key);
        
        if (v == null && sampleType == SampleTypes.FullSample) {
            v = rg.specialValues().get("CA_OriginalProfit_OOS");
            if (v == null) {
                v = rg.specialValues().get("CA_OriginalProfit_IS");
            }
            if (v == null) {
                v = rg.specialValues().get("CA_OriginalProfit");
            }
        }
