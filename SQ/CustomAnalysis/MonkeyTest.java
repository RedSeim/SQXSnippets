...
public class MonkeyTest extends CustomAnalysisMethod {
    ...
    public boolean filterStrategy(String projectName, String task, String databankName, ResultsGroup rg) throws Exception {
        ...
        try {
            ...
            if (tradeCount < 20) {
                if (tradeCount == 0 && sampleType != SampleTypes.FullSample) {
                    Log.warn("MonkeyTest: strategy [" + rg.getName() + "] has no trades in the " + sampleLabel + " period. Verify that the last backtest has that sample period (IS/OOS) configured. -> LOW TRADES.");
                } else {
                    Log.warn("MonkeyTest: strategy [" + rg.getName() + "] has too few trades in the " + sampleLabel + " period (" + tradeCount + " trades, minimum 20). -> LOW TRADES.");
                }
                status = "LOW TRADES";
        ...
        rg.specialValues().setString("MonkeyTestResult", status);
        ...
        rg.specialValues().set("FilterResult", existingFilterResult && passFilters);
