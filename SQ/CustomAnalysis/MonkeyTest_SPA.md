...
4. **Simula la Evaluación del Recorrido**:
   ...
   * **Salidas y Modos de Replicación (`replicationMode`)**:
     * **`SLTP` (Niveles SL/TP)**: Se evalúa vela a vela si el Stop Loss (SL) o el Objetivo de Beneficio (PT) se alcanza primero según las distancias originales. Si el trade original no tenía SL/TP, se utiliza el número de velas de la orden como límite de salida forzoso.
     * **`AvgBars` (Exposición Media Fija - Recomendado)**: Calcula la duración media exacta en barras de la estrategia. Cuando no se utiliza la precisión de 1 minuto (`Precision=M1`), aplica un algoritmo de **Dithering Determinista por Mono** que distribuye las barras fraccionarias en la secuencia de cada mono de forma que **todos y cada uno de los monos simulados tengan de forma garantizada la misma duración media exacta por trade en horas** que la estrategia original.
     * **`IndivBars` (Exposición Individual)**: Aplica a cada trade del mono la duración exacta en barras que tuvo su operación original equivalente.
     * **Recomendación de Uso**: Se recomienda utilizar preferentemente el modo `AvgBars` frente a `IndivBars`, ya que cuenta con una metodología de cálculo más exacta, rigurosa y probada (con reparto determinista insesgado cuando no se usan datos de 1M), garantizando que la exposición temporal media de la estrategia se replique de forma idéntica en cada simulación.
   * **Salida de Viernes**: Cierra automáticamente las operaciones en el umbral de salida de viernes si está definido.
   * **Gestión de Lotaje y Money Management**:
     * **Fixed Size (Lote Fijo)**: El snippet detecta automáticamente si la estrategia opera con un lotaje plano (mediante inspección de los parámetros XML de configuración del proyecto `rg.getLastSettings()` y análisis empírico de las órdenes). En este caso, el mono mantiene **estrictamente el mismo lotaje fijo** (`priceCorrection = 1.0`) en todas las operaciones simuladas.
     * **Fixed Amount / Lotaje Variable**: Si la estrategia utiliza lotajes variables (gestión de riesgo monetario fijo, etc.), el mono aplica la **corrección por retorno porcentual** (`priceCorrection = o.OpenPrice / entryPrice`). Esto adapta la posición simulada en el punto de entrada desplazado para mantener constante el beneficio/pérdida por cada 1% de variación del precio del activo frente a la orden original.
     * **Nota de Alcance y Certificación**: Este Monkey Test se diseñó expresamente para funcionar de manera correcta con métodos de Money Management de **Fixed Size** (Lote Fijo) y **Fixed Amount** (Riesgo Fijo en Dinero). No ha sido testado ni certificado en otros modos de Money Management (como Risk %, Martingala, etc.), por lo que, aunque técnicamente pueda ejecutarse, sus resultados no se encuentran certificados para dichos modos.
...
| **Precision=M1** / **M1** | *(ausente: Main)* | Palabra clave opcional (no posicional o parámetro posicional). Activa la simulación de recorrido de los trades utilizando los datos históricos de velas de 1 minuto (`SYMBOL_M1.dat` o `SYMBOL_1M.dat`). Las entradas de los monos se mantienen en la temporalidad de la estrategia original (ej. H4), pero la duración del trade, Stop Loss, Take Profit y salida de viernes se evalúan a resolución de 1 minuto. Si los datos de 1 minuto no están disponibles en la carpeta `user/data/History/`, el test emite una advertencia en los logs y vuelve automáticamente a la temporalidad principal (`H4`). | `500,95,FULL,IndivBars,Random,Precision=M1` |
| **SegmentDuration=N** | *(ausente: off)* | Palabra clave opcional (ej. `SegmentDuration=300` o `SegmentDuration=200`). Activa una variante del test que divide el periodo analizado en sub-segmentos continuos de duración homogénea ajustados a $N$ días. Se ejecuta de forma adicional e independiente al test principal, publicando los resultados bajo claves específicas (`MonkeyTestResult_Seg_IS_1`, `MonkeyTestResult_Seg_FULL_1`, etc.) y metadatos de recuento/duración. Admite cualquier valor numérico deseado por el usuario. | `500,95,FULL,IndivBars,Random,SegmentDuration=300` |

> **Nota sobre la Precisión de Datos:** La opción de precisión seleccionada en la configuración general del backtest de SQX (ej. la pestaña de motor de backtest en Builder/Retester) **no influye** en la fuente de datos que lee este Custom Analysis. Por defecto, el Monkey Test siempre leerá el fichero del timeframe principal de la estrategia (ej. `H4.dat`). Para forzar al Monkey Test a leer y simular sobre los datos históricos de 1 minuto (`SYMBOL_M1.dat`), es **imprescindible** incluir explícitamente la palabra clave `Precision=M1` o `M1` en la cadena de **Input Args** de la tarea de Custom Analysis.

*Ejemplo de Input Args:* `500,95,OOS2,IndivBars,Random` (Ejecuta 500 monos sobre las operaciones de OOS2, usando salidas de exposición individual por vela y desplazamiento aleatorio por operación; no se escribe ningún fichero de caché). Omitir replicationMode y shiftingMode los establece automáticamente a `IndivBars` y `Random`. Añade `ResultsPluginCache` en cualquier punto de la cadena, p. ej. `500,95,OOS2,IndivBars,Random,ResultsPluginCache`, para además escribir los ficheros de caché del ResultsPlugin. Para ejecutar el test con precisión intrabarra de 1 minuto, añade `Precision=M1` o `M1` en los Input Args (ej. `500,95,FULL,IndivBars,Random,Precision=M1`). Para activar el test segmentado por tiempo, añade `SegmentDuration=300` (ej. `500,95,FULL,IndivBars,Random,SegmentDuration=300`).
...
| `MonkeyTest_SegCount_<PERIODO>` | Metadato: Número total de sub-segmentos creados para el periodo (ej. `MonkeyTest_SegCount_IS = 4`). |
| `MonkeyTest_SegDays_<PERIODO>` | Metadato: Duración real promedio en días de cada sub-segmento (ej. `MonkeyTest_SegDays_IS = 274.0`). |
| `MonkeyTest_SegTargetDays` | Metadato: Días de duración objetivo solicitados en Input Args (ej. `300`). |
